...
( function() {
	...
	function read() {
		try {
			...
			if ( ! v.t || ( Date.now() - v.t ) > DAYS * 86400000 ) {
				return null;
			}
			return v;
		} catch ( e ) {
	...
	function init() {
		...
		if ( ! banner ) {
			return;
		} // markup missing — nothing to wire
		...
		function trap( e ) {
			...
			if ( e.key === 'Escape' && read() ) {
				e.preventDefault(); hideBanner(); return;
			}
			if ( e.key !== 'Tab' ) {
				return;
			}
		...
		function backgroundInert( on ) {
			...
			for ( var i = 0; i < kids.length; i++ ) {
				...
				if ( on ) {
					kids[i].setAttribute( 'inert', '' ); kids[i].setAttribute( 'aria-hidden', 'true' );
				} else {
					kids[i].removeAttribute( 'inert' ); kids[i].removeAttribute( 'aria-hidden' );
				}
			}
		}
		...
		function decide( c ) {
			...
			if ( c.analytics || ( ADS && c.marketing ) ) {
				if ( typeof window.cmv2LoadTag === 'function' ) {
					window.cmv2LoadTag();
				}
			}
			hideBanner();
		...
		function revealPrefs() {
			...
			if ( stored ) {
				if ( catA ) {
					catA.checked = !! stored.c.analytics;
				}
				if ( catM ) {
					catM.checked = !! stored.c.marketing;
				}
			}
			if ( catA ) {
				try {
			...
			} // move focus into the revealed group
		}
		...
		window.cmv2Consent = { open: function() {
			openManager();
		} };
